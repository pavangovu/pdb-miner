...
df %>% group_by(halide, model_name) %>% select(model_name, asa, asa_diff, halide) %>% slice(1) %>% 
  ggplot(aes(x=halide, y=asa_diff, fill=halide))+
  geom_violin()+
  theme_bw()

df %>% group_by(halide, model_name, id) %>% summarise(count=n(), asa_diff=mean(asa_diff)) %>% 
  ggplot(aes(x=sqrt(asa_diff**2), y=count, fill=halide, col=halide))+
  geom_point()+
  geom_smooth()+
  facet_wrap(~halide)+
  ylim(0,10)+
